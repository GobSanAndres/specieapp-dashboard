...
function adapterData(arrayData:any[]) {
  const newData = arrayData.map((agriculturalPrices) => {
    const adapter = {
      ...
      business_address_establishment: agriculturalPrices.business_address_establishment,
...
function headersToData() {
  const headers = ['Nombre del negocio', 'Dirección del negocio', 'Creado por', 'Fecha de creación', 'identificación', 'nombre del informante', 'Localización', 'Nit', 'Tipo de persona', 'Número de registro', 'Fecha de actualización'];
  return headers;
}
