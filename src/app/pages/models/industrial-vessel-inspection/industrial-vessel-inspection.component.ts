  ...
  getIndustrialVesselInspection(status:boolean) {
    ...
    this.industrialVesselInspectionService.getIndustrialVesselInspection(status).subscribe(
      (result: any) => {
        ...
        this.industrialVesselInspectionService.getSpecies(true).subscribe(
          (efect: any) => {
          ...
          (error) => {
            if (error.status === 401) {
              ...
              this.swal.genericError(error.error.message);
            }
          },
        );

        this.industrialVesselInspectionService.getBoat(true).subscribe(
          (efect: any) => {
        ...
        this.industrialVesselInspectionService.getUsers(true).subscribe(
          (efect: any) => {
        ...
        this.industrialVesselInspectionService.getFishingArea(true).subscribe(
          (efect: any) => {
        ...
        this.industrialVesselInspectionService.getFishingSite(true).subscribe(
          (efect: any) => {
  ...
  */
  initForms() {
    this.formEditIndustrialVesselInspection = this.formBuilder.group({
      ...
      patent_number: ['', [Validators.required, Validators.minLength(1)]],
      ...
      // BUCEO
      diving_number_pangas_perday: ['', [Validators.min(0)]],
      diving_number_divers_perpanga: ['', [Validators.min(0)]],
      diving_effort_diver_perday: ['', [Validators.min(0)]],

      // LOBSTERS
      lobster_pots_number_lines_perdays: ['', [Validators.min(0)]],
      lobster_pots_number_lingadas_perlines: ['', [Validators.min(0)]],
      lobster_pots_number_nasa_perlingada: ['', [Validators.min(0)]],
      lobster_pots_effort: ['', [Validators.min(0)]],

      // FISH POTS
      fish_pots_type: ['', [Validators.minLength(1)]],
      fish_pots_number_pots_perday: ['', [Validators.min(0)]],
      fish_pots_effort_pots_perday: ['', [Validators.min(0)]],
      ...
      signatureCaptainOrRepresentative: ['', [Validators.required, Validators.minLength(4)]],
      observations: [''],
  ...
  openModalEdit(editIndustrialVesselInspectionModal:NgbModal, industrialVesselInspection, pos) {
    ...
    if (pos === -4) {
      (<HTMLInputElement> document.getElementById('captain-signature')).setAttribute('src', this.formEditIndustrialVesselInspection.get('signatureCaptainOrRepresentative').value);
      (<HTMLInputElement> document.getElementById('register-signature')).setAttribute('src', this.formEditIndustrialVesselInspection.get('signatureRegister').value);
    }
  }
   ...
   * predeterminado
   */
  updateDataIndustrialVesselInspection() {
    ...
    if (this.formEditIndustrialVesselInspection.invalid) {
      ...
      const dataForm: any = this.formEditIndustrialVesselInspection.value;
  ...
  changeStatus(industrialVesselInspection) {
    ...
    this.industrialVesselInspectionService
      .disableIndustrialVesselInspection({
        ...
        (result: any) => {
          this.swal.notificationLoading(result.message);
          this.getIndustrialVesselInspection(true);
        },
        (error) => {
          if (error.status === 401) {
  ...
  openModalEditEffortCharacteristics(data) {
    ...
    modalRef.result.then((result: any) => {
      // console.log('Editado');
      this.formEditIndustrialVesselInspection.patchValue({
        long_line: result.long_line,
      });

      this.updateDataIndustrialVesselInspection();
    }, (reason: any) => {
      // console.log('No Editado');
    });
  }
  ...
  openModalEditSampleLobster() {
    ...
    modalRef.result.then((result: any) => {
      // console.log('Editado');
      this.formEditIndustrialVesselInspection.patchValue({
        control_total_bags_landed: result.control_total_bags_landed,
        control_total_sealed_bags: result.control_total_sealed_bags,
      });
    }, (reason: any) => {
      // console.log('No Editado');
    });
