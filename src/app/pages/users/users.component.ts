...
export class UsersComponent implements OnInit {
   ...
   */
  filterUsers(event:any, type:string) {
    const wordFilter:string = event.srcElement.value;

    if (type === 'email') {
      (<HTMLInputElement>document.getElementById('search_rol')).value = '';
      (<HTMLInputElement>document.getElementById('search_name')).value = '';
      ...
      (<HTMLInputElement>document.getElementById('search_email')).value = '';
      (<HTMLInputElement>document.getElementById('search_rol')).value = '';
      this.usersArray = this.usersArrayForFilter.filter((userArray) => userArray.name
        .toLocaleLowerCase().indexOf(wordFilter.toLocaleLowerCase()) !== -1);
    } else {
      (<HTMLInputElement>document.getElementById('search_email')).value = '';
      (<HTMLInputElement>document.getElementById('search_name')).value = '';
      this.usersArray = this.usersArrayForFilter.filter((userArray) => userArray.rol.name
        .toLocaleLowerCase().indexOf(wordFilter.toLocaleLowerCase()) !== -1);
    }
  }
    ...
    if (this.formUsers.invalid) {
      ...
      this.userService.createUser(this.formUsers.value).subscribe(
        ...
        (error) => {
          ...
          } else if (error.error.error.keyPattern.email === 1) {
            Swal.fire({
              title: 'Error!!',
              icon: 'error',
              html: 'Correo ya existente',
              showCancelButton: false,
              showConfirmButton: true,
              confirmButtonText: 'Aceptar',
              allowOutsideClick: false,
