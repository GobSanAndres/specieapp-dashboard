  ...
  */
  ...
  saveChangeProduct() {
    ...
    if (this.formEditProduct.invalid) {
      ...
      const hoy = new Date();
      const dateParse = `${this.formEditProduct.value.data} ${hoy.getHours()}:${hoy.getMinutes()}:${hoy.getSeconds()}`;
      this.formEditProduct.patchValue({
        data: new Date(dateParse).getTime(),
      });
  ...
  openModalProductTransshipment(bringModal, posIncome) {
    ...
    this.formEditProduct.patchValue({
      ...
      data: this.transformEpoch(this.dataForm.value.transshipment_product_control[posIncome].data),
