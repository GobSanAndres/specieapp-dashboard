...
function adapterData(arrayData:any[]) {
  const newData = arrayData.map((agriculturalProducer) => {
    const adapter = {
      business_name_establishment: agriculturalProducer.business_name_establishment,
      business_address_establishment: agriculturalProducer.business_address_establishment,
...
function headersToData() {
  const headers = ['Nombre del negocio', 'Dirección del negocio', 'Creado por', 'Fecha de creación', 'Sistema de cultivación', 'Formato de tenencia', 'Genero', 'Identificación', 'Generaración de ingresos', 'Nombre del informante', 'Latitud', 'Longitud', 'Localización', 'Nit', 'Tipo de persona', 'Actividad principal', 'Número de registro', 'Autoconsumo', 'Fecha de actualización', 'Fuente de agua'];
  return headers;
}
