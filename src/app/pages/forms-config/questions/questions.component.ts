...
export class QuestionsComponent implements OnInit {
   ...
   */
  filterQuestion(event:any, type) {
    const wordFilter:string = event.srcElement.value;
    if (type === 'quest') {
      (<HTMLInputElement>document.getElementById('search_sect')).value = '';
      (<HTMLInputElement>document.getElementById('search_desc')).value = '';
      ...
      (<HTMLInputElement>document.getElementById('search_sect')).value = '';
      (<HTMLInputElement>document.getElementById('search_quest')).value = '';
      this.questionArray = this.questionArrayForFilter
        .filter((questionArray) => questionArray.description
          .toLocaleLowerCase().indexOf(wordFilter.toLocaleLowerCase()) !== -1);
    } else if (type === 'sect') {
      (<HTMLInputElement>document.getElementById('search_quest')).value = '';
      (<HTMLInputElement>document.getElementById('search_desc')).value = '';
      ...
      this.questionArray = this.questionArrayForFilter
        .filter((questionArray) => questionArray.section_form.description
          .toLocaleLowerCase().indexOf(wordFilter.toLocaleLowerCase()) !== -1);
    }
  }
  ...
  createArraysQuestion(name, tipo) {
    ...
    switch (name) {
      ...
      case 'status':
        ...
        break;
      case 'long_line':
        array = [{ idData: 'Pelagico', descriptionData: 'Pelágico' }];
        array.push({ idData: 'Demersal', descriptionData: 'Demersal' });
        array.push({ idData: 'No_aplica', descriptionData: 'No/aplica' });
        break;
      case 'type_fishshop':
      ...
      case 'specie':
        ...
        array2.forEach((element) => {
          if (iterator === 0) {
            array = [{ idData: element.scientific_name, descriptionData: element.scientific_name }];
          } else {
            array.push({ idData: element.scientific_name, descriptionData: element.scientific_name });
          }
          iterator += 1;
        });
        break;
